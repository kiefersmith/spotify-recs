...
class Actions:
    def __init__(self, get, save, load, user, playlist_list, spotipy):
        self.g = get
        self.s = save
        self.l = load
        self.user = user
    ...
    def get_recommendations(self):
        ...
        for name in self.playlist_list:
            print(name)
            tracks_dict[name] = []
            rows = df.loc[df['name'].isin(self.playlist_list)]
        ...
        for name in self.playlist_list:
            ...
            recs = self.g.recommendations(tracks_dict[name])
